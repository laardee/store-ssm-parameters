...
function createPaths(object, pathPart = '') {
  return _.flattenDeep(_.map(object, (value, key) => {
...
function storeParameter(parameter) {
  return new Promise((resolve) => {
      ...
      return setTimeout(() => {
        function store() {
          return ssm.putParameter(_.assign({ Overwrite: true }, parameter)).promise()
          .then(result => {
            console.log('Parameter stored (Version %s)', result.Version);
            return resolve(result);
          });
        }
        return ssm.getParameter({ Name: parameter.Name, WithDecryption: true }).promise()
...
function storeParameters(parameters) {
  ...
  return parameterPaths.map(parameter =>
    queue.add(() =>
